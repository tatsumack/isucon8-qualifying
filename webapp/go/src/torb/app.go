...
func getEvent(eventID, loginUserID int64) (*Event, error) {
	...
	for _, p := range sheets {
		...
		if ok {
			...
			sheet.Reserved = true
			sheet.ReservedAtUnix = res.ReservedAt.Unix()
		} else {
			event.Remains++
...
func main() {
	...
	e.GET("/api/users/:id", func(c echo.Context) error {
		...
		rows, err := db.Query("SELECT * FROM reservations WHERE user_id = ? ORDER BY IFNULL(r.canceled_at, r.reserved_at) DESC LIMIT 5", user.ID)
		if err != nil {
			return err
		}
		defer rows.Close()
	...
	e.DELETE("/api/events/:id/sheets/:rank/:num/reservation", func(c echo.Context) error {
		...
		if err := tx.QueryRow("SELECT * FROM reservations WHERE id = ? FOR UPDATE", reservation.ID).Scan(&reservation.ID, &reservation.EventID, &reservation.SheetID, &reservation.UserID, &reservation.ReservedAt, &reservation.CanceledAt); err != nil {
			tx.Rollback()
			if err == sql.ErrNoRows {
				return resError(c, "not_reserved", 400)
			}
